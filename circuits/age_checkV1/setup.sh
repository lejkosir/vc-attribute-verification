...
snarkjs powersoftau contribute "$CIRCUIT_DIR/pot12_0.ptau" \
    "$CIRCUIT_DIR/pot12_1.ptau" --name="dev" -e="$(openssl rand -hex 32)"
...
mkdir -p "$WALLET_DIR"
cp "$CIRCUIT_DIR/${CIRCUIT_NAME}_js/$CIRCUIT_NAME.wasm"    "$WALLET_DIR/age_check.wasm"
cp "$CIRCUIT_DIR/${CIRCUIT_NAME}_js/generate_witness.js"   "$WALLET_DIR/generate_witness.js"
cp "$CIRCUIT_DIR/${CIRCUIT_NAME}_js/witness_calculator.js" "$WALLET_DIR/witness_calculator.js"
cp "$CIRCUIT_DIR/${CIRCUIT_NAME}_final.zkey"               "$WALLET_DIR/age_check_final.zkey"
